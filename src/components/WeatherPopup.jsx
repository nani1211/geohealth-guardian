...
/**
 * WeatherPopup — floating popup rendered over the map on click.
 * Shows: temperature, humidity, wind, condition, health advisory, address + coords.
 * Props:
 *  • weather     — { temp, humidity, windSpeed, description, condition, lat, lon, name }
 *  • address     — { city, region, formatted }
 *  • screenPoint — { x, y } pixel coordinates of the click
 *  • tempUnit    — '°C' | '°F'
 *  • windUnit    — 'm/s' | 'mph'
...
const WeatherPopup = ({ weather, address, screenPoint, tempUnit, windUnit, onClose, loading, lat, lon }) => {
  ...
  // Show small loading state before data comes in
  if (!weather && loading) {
    const style = { left: `${screenPoint.x}px`, top: `${screenPoint.y + 20}px`, transform: 'translateX(-50%)' };
    return (
      <div className="absolute z-50 pointer-events-auto" style={style}>
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 px-5 py-4 flex items-center gap-3">
          <div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
          <span className="text-sm text-gray-600 font-medium">Fetching weather…</span>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 cursor-pointer ml-1"><X size={14} /></button>
        </div>
      </div>
    );
  }

  // If weather is null but finished loading (API error), show location-only card
  ...
  return (
    <div id="weather-popup-debug-container" className="absolute pointer-events-auto" style={{ zIndex: 9999, ...style }}>
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-80 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {hasWeather && (
                <p className="text-blue-200 text-[10px] capitalize truncate">
        ...
        {/* Stats grid — only shown when weather available */}
